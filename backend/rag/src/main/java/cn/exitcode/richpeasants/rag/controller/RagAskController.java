...
public class RagAskController {
    ...
    @PostMapping("/ask")
    public ApiResult<RagAskResponse> ask(@AuthenticationPrincipal LoginUser loginUser,
                                         @Valid @RequestBody RagAskRequest request) {
        return ApiResult.ok(ragAskService.ask(request, loginUser));
    }
    ...
    public SseEmitter askStream(@AuthenticationPrincipal LoginUser loginUser,
        ...
        CompletableFuture.runAsync(() -> ragAskService.askStream(request, loginUser, emitter));
        return emitter;
    }
}
