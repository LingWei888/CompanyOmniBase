export interface ProblemConvertPayload {
  modelId: number
...
export interface ProblemConvertStreamHandlers {
  onDelta?: (content: string) => void
  onDone?: (markdown: string) => void
  onError?: (message: string) => void
}

export async function convertProblemStream(
  payload: ProblemConvertPayload,
  handlers: ProblemConvertStreamHandlers,
) {
  const userToken = localStorage.getItem('user_access_token')
  if (!userToken) {
    throw new Error('请先登录后再使用题意修改智能体')
  }
  ...
  const response = await fetch('/api/auth/agents/problem-convert/stream', {
    method: 'POST',
    headers,
    ...
    if (eventName === 'delta') {
      ...
      const markdown =
        data && typeof data === 'object' && 'markdown' in data
          ...
          : ''
      // 支持 async onDone；完成后即可结束流读取，避免连接迟迟不关导致一直「转换中」
      const result = handlers.onDone?.(markdown) as void | Promise<void>
      ...
      const message =
        data && typeof data === 'object' && 'message' in data
          ? String((data as { message?: string }).message || '转换失败')
          : '转换失败'
